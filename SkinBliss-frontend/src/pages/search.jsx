...
export default function SearchPage({ query }) {
  ...
  if (!isLoading && !isError && products.length > 0) {
    ...
    if (searchText && !productType) {
      product_items = all_products.filter((prd) => prd.name?.toLowerCase().includes(searchText?.toLowerCase()));
    }
    if (searchText && productType) {
      product_items = all_products
        .filter((prd) => prd.category.name?.toLowerCase() === productType?.toLowerCase())
